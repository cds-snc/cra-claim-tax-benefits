...
describe('Full run through with edits', function() {
  ...
  beforeEach(() => {
    ...
    cy.fixture('user_edits.json').as('userEdits')
  })
  ...
  it('navigates the Auth page', function() {
    //AUTH PAGE
    cy.injectAxe().checkA11y()
    cy.get('input#auth')
      .clear()
  ...
  it('updates the Mailing Address', function() {
    //EDIT MAILING
    cy.injectAxe().checkA11y()
    cy.url().should('contain', '/personal/address/edit')
    cy.get('input#line1')
      .clear()
      .type(this.userEdits.addressNew.line1)
    ...
    cy.get('input#postalCode')
      .clear()
      .type(this.userEdits.addressNew.postalCode)
  ...
  it('checks the Mailing Address change is reflected', function() {
    //CONFIRM MAILING AGAIN
    cy.injectAxe().checkA11y()
    cy.url().should('contain', '/personal/address')

    //format address based on apartment/no apartment
    const newAddressText = getAddress(this.userEdits.addressNew)
  ...
  it('it changes the Marital Status', function() {
    //EDIT MARITAL STATUS
    cy.injectAxe().checkA11y()
  ...
  it('checks the Marital Status change is reflected', function() {
